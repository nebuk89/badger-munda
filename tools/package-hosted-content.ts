...
const frameStarts = new Map<string, number>()
let nextFrameId = 1
for (const artwork of artworks) {
  if (frameStarts.has(artwork.clip.id)) throw new Error(`Duplicate built-in clip ID: ${artwork.clip.id}`)
  frameStarts.set(artwork.clip.id, nextFrameId)
  nextFrameId += artwork.clip.frameCount
}

async function packageArtwork(staging: string, artwork: Artwork): Promise<HostedClip> {
  const firstFrameId = frameStarts.get(artwork.clip.id)
  if (firstFrameId === undefined) throw new Error(`Unknown built-in clip: ${artwork.clip.id}`)
...
export async function packageHostedContent(
  ...
  try {
    ...
    for (const artwork of selectedArtworks) clips.push(await packageArtwork(staging, artwork))
    const stableCatalog = { version: 1 as const, clips }
    const catalogHash = digest(JSON.stringify(stableCatalog))
    const catalog: HostedCatalog = { ...stableCatalog, catalogHash }
